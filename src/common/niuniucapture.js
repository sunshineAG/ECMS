...
*******************************************************************************/
/**
 * 核心代码已进行对应修改，之前为es5写法，且使用jq
 * by cczhuc 2019/07/16
 * */
/*******************************************************************************/
//设置截图的参数
...
function NiuniuCaptureObject()
{
    ...
    this.LoadPlugin = function()
    {
        ...
        if(!obj)
        {
            document.querySelectorAll('body')[0].insertAdjacentHTML('beforeend','<div id="capturecontainer" style="height:0px;width:0px;"></div>');
        ...
        if(!iframeObj)
        {
            document.querySelectorAll('body')[0].insertAdjacentHTML('beforeend','<iframe id="startCaptureFrame" style="display:none;"></iframe>');
        }
			...
			try{
				...
				self.NiuniuSocket.onmessage = function(msg){
					...
					if(id == "0"){
                        ...
                        self.TimeIntervalID = setInterval(_=>{
	...
	this.OnWebSocketError = function(type)
	{
        ...
        self.TimeIntervalID = setTimeout(_=>{
            self.connectHost()
		},800)
    ...
    this.DoCaptureForCustomize = function(name, hide, AutoCapture, x, y, width, height)
    {
	    ...
	    try
	    {
		    ...
		    else
		    {
		        ...
		        document.getElementById('startCaptureFrame').setAttribute('src', newUrl)
...
export {NiuniuCaptureObject,isMacintosh,rgb2value};
