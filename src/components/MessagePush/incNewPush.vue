<template>
	<div class="companyStyle creatPush" >
		...
		<div class="company-content page-style incPush" >
			<a-form :form="form" id='components-form-demo-normal-login' @submit.prevent="handleSubmit" class='login-form' v-show="!fixed_show">
				...
				<a-row type="flex" justify="start">
					...
					<a-col :span="24" class="col-margin"  v-if="queryForm.visble_type==20||queryForm.visble_type==30" style="padding-left:14px;">
						<a-form-item   :label="pushType=='20'?'选择群':'选择客户'"  v-bind="formItemLayout" style="margin-bottom:12px;">
						...
						<a-form-item  label=" " :colon="false" v-bind="formItemLayout" style="margin-bottom:12px;">
							<div class="ant-alert">
								...
								<span class="ant-info" v-if="userTotal==0?true:false">
									<label style="padding-left:10px;">{{pushType=='20'?'所有群不可见':'所有客户不可见'}} </label>
								</span>
								<span class="ant-info" v-else>
									<label style="padding-left:10px;">已选择</label><span > {{userTotal}} </span><label>{{pushType=='20'?'个群':'个客户'}}(已去重)</label> <span class="pointer" @click="clearLabel">清空</span>
								</span>
							</div>
						</a-form-item>
								...
								<a-col :span="24" style="padding: 0 20px 0 0">
									...
									<template >
										<a-form-item v-for="(k,index) in  sendMsgContent" :required="false" :key="index" class="material_content" style="margin-bottom:2px;width:748px;">
											...
											<a-textarea v-else v-model="k.content" :placeholder="pushType=='10'?'请输入内容':'请输入内容'" :autosize="{'minRows': 4, 'maxRows': 6 }" :maxlength="pushType=='20'?700:500" style="width:748px;" />
											<a-icon
													v-if="index > 0"
                                            ...
                                            <!-- <p v-if="!k.materialItem.length&&pushType=='10'" >在文本中输入标签系统替换对应的类容：<b @click="user(k,'{微信备注名}')" class="tag">{微信备注名}</b>，<b @click="user(k,'{微信昵称}')" class="tag">{微信昵称}</b>，<b @click="user(k,'{手机号}')" class="tag">{手机号}</b>，<b @click="user(k,'{客户生日}')" class="tag">{客户生日}</b></p> -->
                                             <!-- v-if="pushType=='10'" -->
                                        </a-form-item>
			...
			<a-modal :title="pushType=='20'?'选择群':'选择客户'" :footer="null" :visible="visible>1?true:false" @cancel="com_handleCancel" width="900px">
				<a-row>
					<a-col :xl="{span:11}" :lg="{span:11}">
						<a-input-search placeholder="请输入搜索内容。。。" enterButton v-model="searchKey_friend" style="width: 45%" @search="onSearch_friend"  v-if="visible==2?true:false"/>
						<a-input-search placeholder="请输入搜索内容。。。" enterButton v-model="searchKey_user" style="width: 50%" @search="onSearch_user"  v-if="visible==3?true:false" />
						<!-- <a-icon v-if="visible==2&&searchKey_friend!=''" type="close-circle" @click="searchKey_friend=''" style="position: absolute;right: 60px;top: 50%;transform: translateY(-50%);cursor: pointer;" />
						<a-icon v-if="visible==3&&searchKey_user!=''" type="close-circle" @click="searchKey_user=''" style="position: absolute;right: 60px;top: 50%;transform: translateY(-50%);cursor: pointer;" /> -->
                        <span v-if="visible==3" style="color:#FE6354;vertical-align:sub">注意（仅展示有{{pushType=='20'?'群':'客户'}}的标签）</span>
                        <!-- <span v-if="pushType=='20'" style="color:#FE6354;vertical-align:sub;font-size:12px;">注意 ( 群聊包含多个员工号只展示一个 ) </span> -->
					</a-col>
					<a-col :xl="{span:13}" :lg="{span:13}" class="pagination_box" style="text-align:right">
                ...
                <a-table  v-if="visible==3" :columns="pushType=='10'?columnsTag1:columnsTag" :dataSource="userLabel" :locale="locale"  :pagination="false" type="checkbox"  :rowSelection="{selectedRowKeys: modalSelectedRowKeys, onChange: onSelectChange}" :rowKey="(record) => record.id"  :rowClassName="(record, index) => {return index % 2 === 0 ? 'bg-row' : 'bg-row-gray'}" >
                    <template  slot="tagName" slot-scope="text,record,index">
                        <div>
                            <span :class="['title-tagColor'+record.color_id]" style="border-radius:100%;padding:0 9px;margin-right:6px;"></span>
                            <span>{{record.title}}</span>
                        </div>
                    </template>
                </a-table>
                <a-table  v-if="visible==2" :columns="pushType=='10'?columns1:columns" :dataSource="friendsId" :locale="locale"  :pagination="false" type="checkbox"  :rowSelection="{selectedRowKeys: modalSelectedRowKeys1, onChange: onSelectChangeCustorm}" :rowKey="(record) => record.id"  :rowClassName="(record, index) => {return index % 2 === 0 ? 'bg-row' : 'bg-row-gray'}" >
                     <template v-if="pushType=='20'" slot="assistant_avatar" slot-scope="text,record,index">
                         <div>
                            <img src='../../assets/roomDefalt.svg' alt="" style="width: 30px;height: 30px;border-radius:100%">
                            <span>{{record.nickname||record.real_name ||'未命名的群'}}</span>
                         </div>
                     </template>
                    <template v-if="pushType=='10'" slot="avatar_url" slot-scope="text,record,index">
                       <div>
                            ...
                            <span>{{record.nickname||record.real_name}}</span>
                       </div>
                    </template>
                    <template  v-if="pushType=='10'" slot="robot_username" slot-scope="text,record,index">
                       <div>
                            ...
                            <span>{{record.assistant_name||record.adviser_name}}</span>
                       </div>
                    </template>
                </a-table>
				<a-form >
           ...
           <a-modal :footer="null" :visible="visible==1?true:false" @cancel="com_handleCancel('3')" width="80%" >
				...
				<a-row >
					<a-col :xl="{span:11}" :lg="{span:12}">
					</a-col>
					<a-col :xl="{span:13}" :lg="{span:24}" class="pagination_box" style="margin-bottom: 0;position: relative;z-index: 10">
				...
				<a-form :form="form" class="useMaterial inc">
					<a-checkbox-group  @change="sel_Material" v-model="modalIsChecked" style="margin: 0 5px 0 0;width: 100%;" class="checkbox_content sel_material">
						<a-tabs @change="tab_callback" type="card" class="modal_tabs" :activeKey="activeKey">
							...
							<a-tab-pane tab="链接" key="3">
                                <data-status v-if="!linkData.length" :width="62" :material="true" style="margin-top:48px;"></data-status>
								<a-list v-else  :grid="{ gutter: 16, xl: 6,lg:4,md: 4, sm: 2, xs: 1}" :dataSource="linkData">
									<a-list-item slot="renderItem" slot-scope="item, index">
										<a-checkbox :value="item.id" style="width: 100%;">
							...
							<a-tab-pane tab="名片" key="4" >
                                    <data-status v-if="!linkData.length" :width="62" :material="true" style="margin-top:48px;"></data-status>
									<a-list v-else :grid="{ gutter: 16, xl: 6,lg:4,md: 4, sm: 2, xs: 1}" :dataSource="cardData">
										<a-list-item slot="renderItem" slot-scope="item, index">
											<a-checkbox :value="item.id" style="width: 100%;">
							...
							<a-tab-pane tab="小程序" key="5" >
                                    <data-status v-if="!linkData.length" :width="62" :material="true" style="margin-top:48px;"></data-status>
									<a-list v-else :grid="{ gutter: 16, xl: 6,lg:4,md: 4, sm: 2, xs: 1}" :dataSource="programData">
										<a-list-item slot="renderItem" slot-scope="item, index">
											<a-checkbox :value="item.id" style="width: 100%;">
							...
							<!-- <a-tab-pane tab="朋友圈素材" key="6" v-if="pushType=='20'">
                            ...
                            <a-tab-pane tab="视频素材" key="7" >
                                    <data-status v-if="!linkData.length" :width="62" :material="true" style="margin-top:48px;"></data-status>
									<a-list v-else :grid="{ gutter: 16, xl: 6,lg:4,md: 4, sm: 2, xs: 1}" :dataSource="videoData">
										<a-list-item slot="renderItem" slot-scope="item, index">
											<a-checkbox :value="item.id" style="width: 100%;">
												<a-card hoverable class="link_box push-friend" style="border-radius: 5px">
		...
		data() {
			...
			return {
                ...
                columns:[
                    ...
                    width: "75%",
                    align:'left',
                    dataIndex:'assistant_avatar',
                    scopedSlots:{
                        customRender:'assistant_avatar'
                    }
                    },
                    {
                    title:'群类型',
...
//                        return text==='0'?'普通':text==='1'?'全员':text==='2'?'外部群':text==='3'?'内部群':text==='4'?'小程序':'BOT'
                    }
                    }
                ],
                columns1:[
...
//					friendsChecked: false, //true-排除当前选择--机器人
		...
		methods: {
			...
			select_user_onShowSizeChange(page, pageSize){//标签分页
...
//			},
			...
			async handleSubmit(e) {//提交
				...
				this.form.validateFields((err, fieldsValue) => {
					...
					if(that.queryForm.sendValue == 10&&this.pushTimeShow.length==0) {//10 定时发送 20 立即发送
                        this.$message.warning('必须选择发送时间');
                        return
...
//                    timePicker = [...new Set(timePicker)]
                    ...
                    this.select_user.forEach(el=>{
                        select_filter.push(el.id)
                    })
					this.$axios({
						method: 'POST',
						url: '/index.php?r=uc/enterprise/push-add',
...
//                            'department_id': that.queryForm.department_id,
                            ...
                            'friendsId':select_filter,//选中的客户或群的id
                            'talk_type': that.pushType==='10'?0:10,
...
//						that.text_imgData = !!udata.data.list.text_imgData?udata.data.list.text_imgData:[];
						that.materialDataSelect = that.materialDataSelect.concat(that.textData,that.imgData,that.linkData,that.cardData,that.programData,that.text_imgData,that.videoData)
			...
			getUserList(){//获取用户
				...
				this.$axios({
					method: 'GET',
					url: '/index.php?r=uc/enterprise/list',
					params: {
                        ...
                        'type':1,
                        'talk_type': this.pushType != '10'?'10':'0'
					}
				}).then(function(d) {
					// 响应成功回调
					var udata = d.data;
			...
			refreshGroup() {//请求标签
				...
				this.$axios({
					method: 'POST',
					url: '/index.php?r=uc/enterprise/customer-label',
					data:{
					    'assistant_id':that.selectWxlist,
						'department_id': that.queryForm.department_id,
                    ...
                    var udata = d.data;
					if(udata.status == 1) {
                        that.label_pagination.total =udata.data.total?parseInt(udata.data.total):0; //数据总条数
			...
			getLabelTotal() {//请求用户总数
				...
				this.$axios({
					method: 'POST',
					url: '/index.php?r=uc/enterprise/label-friends-num',
					data: Object.assign({},{
                        'assistant_id':that.selectWxlist,
						'department_id': that.queryForm.department_id,
						'userLabel':lable,
						'friendsId':that.select_user,
...
//                this.queryForm.department_id
                this.$axios({
                    method: 'POST',
                    url: '/index.php?r=uc/enterprise/department-assistant-list',
                    data:{
			...
			getDisabledHours(current){
				if(this.timeFlag){
					...
					hours.splice(new Date().getHours(), 10);
