	        ...
	        <a-list-item slot="renderItem" slot-scope="item, index" class="card-box">
                ...
                <div class="textMaterial"  :hoverable="true" @click="methodHiden ? choseFn(item) : ''" :class="[methodHiden?'tempActive':'']" >
                    <div  style="width: 100%; height: 100%;overflow:hidden" >
                        <video  style="width:100%;" id="upVideo">
                     ...
                     <div class="cover_box"  slot="cover" v-if ="methodHiden ? false : true" >
                        ...
                        <a-icon type="caret-right" class="btn" @click="play(item)"/>
                        <span class="btn">
		...
		methods: {
			editFn(item){
				 this.$router.push({
          			...
          			query: {'id':item.id,'tabs_key':8,'ask':item.ask}
        		})
            },
            play(item){
                this.previewVideo = true;
                this.videoPath = item.mediapath;
            },
            choseFn(id){
                console.log(id)
            ...
            findVideoCover(){
                ...
                var video =  document.createElement('video')
                    ...
                    this.dataSource.forEach(element => {
                        video.src= element.mediapath;
                        video.type="video/mp4"
                        ctx.drawImage(video,20,20,20,20);
                        console.log(ctx)
                    });
            },
		    cancel (e) {
